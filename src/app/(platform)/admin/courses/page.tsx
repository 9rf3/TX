          ...
          {filtered.map((course) => (
            <motion.div key={course.id} variants={item}>
              <Card className="flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-xl shrink-0" style={{ background: course.gradient || "#1e293b" }}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex gap-1">
                    <Link href={`/courses/${course.id}`} className="p-2 rounded-lg hover:bg-white/5 text-muted hover:text-foreground transition-colors">
                      <Eye className="w-4 h-4" />
                    </Link>
                    <Link href={`/admin/courses/${course.id}/edit`} className="p-2 rounded-lg hover:bg-white/5 text-muted hover:text-foreground transition-colors">
                      <Edit className="w-4 h-4" />
                    </Link>
                    <button onClick={() => handleDelete(course.id, course.title)} className="p-2 rounded-lg hover:bg-accent-red/10 text-muted hover:text-accent-red transition-colors cursor-pointer">
